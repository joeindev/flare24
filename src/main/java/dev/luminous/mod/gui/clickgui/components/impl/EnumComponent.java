package dev.flare24.mod.gui.clickgui.components.impl;

import dev.flare24.Flare;
import dev.flare24.api.utils.math.Animation;
import dev.flare24.mod.modules.impl.client.ClickGui;
import dev.flare24.mod.modules.settings.impl.EnumSetting;
import dev.flare24.core.impl.GuiManager;
import dev.flare24.api.utils.render.Render2DUtil;
import dev.flare24.api.utils.render.TextUtil;
import dev.flare24.mod.gui.clickgui.components.Component;
import dev.flare24.mod.gui.clickgui.ClickGuiScreen;
import dev.flare24.mod.gui.clickgui.tabs.ClickGuiTab;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : Flare.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : Flare.GUI.getColor());
		}
