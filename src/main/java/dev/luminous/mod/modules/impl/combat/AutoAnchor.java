...
import dev.flare24.api.utils.math.AnimateUtil;
...
import static dev.flare24.api.utils.world.BlockUtil.*;
...
public class AutoAnchor extends Module {
	...
	public AutoAnchor() {
		...
		setChinese("自动重生锚");
		INSTANCE = this;
		Flare.EVENT_BUS.subscribe(new AnchorRender());
	}
	...
	public void onUpdate() {
		...
		if (currentPos != null) {
			...
			boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || Flare.BREAK.isMining(currentPos, false));
			if (shouldSpam) {
	...
	public BlockPos getHelper(BlockPos pos) {
		for (Direction i : Direction.values()) {
			if (checkMine.getValue() && Flare.BREAK.isMining(pos.offset(i))) continue;
