...
import dev.flare24.api.events.impl.MoveEvent;
...
import dev.flare24.api.utils.entity.MovementUtil;
...
public class Surround extends Module {
    ...
    private void tryPlaceBlock(BlockPos pos) {
        if (pos == null) return;
        if (detectMining.getValue() && Flare.BREAK.isMining(pos)) return;
    ...
    public BlockPos getHelperPos(BlockPos pos) {
        for (Direction i : Direction.values()) {
            if (detectMining.getValue() && Flare.BREAK.isMining(pos.offset(i))) continue;
