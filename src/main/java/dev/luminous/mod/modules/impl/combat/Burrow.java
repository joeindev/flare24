...
import static dev.flare24.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (Flare.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            Flare.ROTATION.snapAt(Flare.ROTATION.rotationYaw, 90);
        }
    ...
    private void gotoPos(BlockPos offPos) {
        ...
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, Flare.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && Flare.BREAK.isMining(pos)) {
            return false;
        }
