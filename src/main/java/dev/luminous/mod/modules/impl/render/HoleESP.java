...
public class HoleESP extends Module {
	...
	Type isHole(BlockPos pos) {
		if (mc.world.isAir(pos) && (!airYCheck.getValue() || pos.getY() == mc.player.getBlockY() - 1) && Flare.HOLE.isHard(pos.up())) return Type.Air;
		int blockProgress = 0;
		boolean bedRock = true;
		for (Direction i : Direction.values()) {
			if (i == Direction.UP || i == Direction.DOWN) continue;
			if (Flare.HOLE.isHard(pos.offset(i))) {
		...
		if (Flare.HOLE.isDoubleHole(pos)) return Type.Normal;
		return Type.None;
	}
