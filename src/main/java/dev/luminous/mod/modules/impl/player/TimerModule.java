...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !Flare.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = Flare.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					Flare.TIMER.set((float) Math.max(Flare.TIMER.getDefault(), timer));
				} else {
					Flare.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
