...
import dev.flare24.mod.modules.settings.Placement;
...
public class AutoTrap
    ...
    private void trapTarget(PlayerEntity target) {
        if (onlyHole.getValue() && !Flare.HOLE.isHole(EntityUtil.getEntityPos(target))) return;
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (antiStep.getValue() && (Flare.BREAK.isMining(pos.up(2)) || !onlyBreak.getValue())) {
        ...
        if (chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (!onlyBreaking.getValue() || Flare.BREAK.isMining(pos.up(2))) {
                    tryPlaceObsidian(offsetPos);
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand, boolean web) {
        ...
        if (Flare.BREAK.isMining(pos)) return;
