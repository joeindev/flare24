...
import dev.flare24.api.events.impl.LookAtEvent;
...
import dev.flare24.api.utils.math.*;
import dev.flare24.api.utils.render.ColorUtil;
import dev.flare24.api.utils.render.JelloUtil;
...
import dev.flare24.asm.accessors.IEntity;
import dev.flare24.Flare;
import dev.flare24.mod.modules.Module;
import dev.flare24.mod.modules.impl.client.AntiCheat;
import dev.flare24.mod.modules.impl.client.ClientSetting;
import dev.flare24.mod.modules.impl.exploit.Blink;
import dev.flare24.mod.modules.impl.player.PacketMine;
import dev.flare24.mod.modules.impl.render.ExplosionSpawn;
...
import static dev.flare24.api.utils.world.BlockUtil.getBlock;
import static dev.flare24.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        Flare.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || Flare.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
    ...
    private boolean faceVector(Vec3d directionVec) {
        if (!yawStep.getValue()) {
            ...
            if (Flare.ROTATION.inFov(directionVec, fov.getValueFloat())) {
                return true;
            }
        }
