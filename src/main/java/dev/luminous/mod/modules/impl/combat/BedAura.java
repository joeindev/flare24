...
import dev.flare24.api.events.impl.RotateEvent;
import dev.flare24.api.events.impl.UpdateWalkingPlayerEvent;
import dev.flare24.api.utils.combat.CombatUtil;
import dev.flare24.api.utils.entity.EntityUtil;
import dev.flare24.api.utils.entity.InventoryUtil;
import dev.flare24.api.utils.math.ExplosionUtil;
import dev.flare24.api.utils.math.FadeUtils;
import dev.flare24.api.utils.math.Timer;
import dev.flare24.api.utils.render.ColorUtil;
import dev.flare24.api.utils.render.Render3DUtil;
import dev.flare24.api.utils.world.BlockUtil;
import dev.flare24.Flare;
import dev.flare24.mod.modules.Module;
import dev.flare24.mod.modules.settings.SwingSide;
import dev.flare24.mod.modules.settings.impl.BooleanSetting;
import dev.flare24.mod.modules.settings.impl.ColorSetting;
import dev.flare24.mod.modules.settings.impl.EnumSetting;
...
public class BedAura extends Module {
	...
	public void onEnable() {
		lastYaw = Flare.ROTATION.lastYaw;
		lastPitch = Flare.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(Flare.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !Flare.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !Flare.BREAK.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(Flare.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(Flare.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			Flare.ROTATION.lookAt(directionVec);
			return true;
		} else {
			this.directionVec = directionVec;
			if (Flare.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
