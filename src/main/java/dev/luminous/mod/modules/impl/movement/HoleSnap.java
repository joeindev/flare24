...
public class HoleSnap extends Module {
   ...
   private final SliderSetting priority = add(new SliderSetting("Priority", 10,0 ,100, grim::getValue));
    ...
    public void onEnable() {
        ...
        resetMove = false;
        holePos = Flare.HOLE.getHole((float) range.getValue(), true, any.getValue(), up.getValue());
    }
    ...
    public void onUpdate() {
        ...
        if (Flare.HOLE.isDoubleHole(holePos)) {
            Direction facing = Flare.HOLE.is3Block(holePos);
            if (facing != null) {
